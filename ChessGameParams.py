...
class TkinterGameSetupParams:
    ...
    def __init__(self):
        ...
        Radiobutton(self.frame, text="Human",variable=self.tk_player2Type,value="human", command=self.toggle_remote).grid(row=3,column=2)
        Radiobutton(self.frame, text="Random AI",variable=self.tk_player2Type,value="randomAI", command=self.toggle_remote).grid(row=3,column=3)
        Radiobutton(self.frame, text="Defense AI",variable=self.tk_player2Type,value="defenseAI", command=self.toggle_remote).grid(row=3,column=4)
        Radiobutton(self.frame, text="Offense AI",variable=self.tk_player2Type,value="offenseAI", command=self.toggle_remote).grid(row=3,column=5)
        Radiobutton(self.frame, text="Remote",variable=self.tk_player2Type,value="remote", command=self.toggle_remote).grid(row=3,column=6)
        self.tk_player2Type.set("defenseAI")

        Label(self.frame, text="Remote host address").grid(row=4,column=0)
        self.entry_remote = Entry(self.frame)
        self.entry_remote.grid(row=4,column=1)
        self.entry_remote.insert(ANCHOR,"localhost")
        self.entry_remote.configure(state="disabled")
    ...
    def toggle_remote(self):
        curr_selection = self.tk_player2Type.get()

        if curr_selection == "remote":
            self.entry_remote.configure(state="normal")
            self.entry_player2Name.configure(state="disabled")
        else:
            self.entry_remote.delete(0, END)
            self.entry_remote.configure(state="disabled")
            self.entry_player2Name.configure(state="normal")
    ...
    def ok(self):
        ...
        self.remoteHost = self.entry_remote.get()
    ...
    def GetGameSetupParams(self):
        ...
        return (self.player1Name, self.player1Color, self.player1Type, 
                self.player2Name, self.player2Color, self.player2Type, self.remoteHost)
