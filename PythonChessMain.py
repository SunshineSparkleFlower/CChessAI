...
class PythonChessMain:
    def __init__(self,options):
        ...
        if options.debug:
            self.Board = ChessBoard(2, self.Rules)
            self.debugMode = True
        else:
            self.Board = ChessBoard(0, self.Rules)#0 for normal board setup; see ChessBoard class for other options (for testing purposes)
    ...
    def MainLoop(self):
        ...
        print "\b\b# " + ("1-0" if currentColor == "white" else "0-1")
