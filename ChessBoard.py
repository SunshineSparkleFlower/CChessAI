...
class ChessBoard:
    def __init__(self,setupType, rules):
        self.Rules = rules
    ...
    def getMessageString(self, piece, moveTuple, capture, pawnPromotion, isInCheck):
        fromt = self.ConvertToAlgebraicNotation(moveTuple[0])
        tot = self.ConvertToAlgebraicNotation(moveTuple[1])
        conv_list = {
            'P': '',
            'R': 'R',
            'T': 'N',
        ...
        if capture and piece[-1] == "P":
            capture = fromt[0].lower() + "x"
        elif capture:
            capture = "x"
        else:
            capture = ""

        return conv_list[piece[-1]] + capture + tot + ("=Q" if pawnPromotion else "") + ("+" if isInCheck else "")
    ...
    def MovePiece(self,moveTuple):
        ...
        isInCheck = self.Rules.IsInCheck(self.GetState(), "white" if fromPiece[0].lower() == "b" else "black")
        ...
        pawnPromotion = False
        if fromPiece == "wP" and toSquare_r == 0:
            self.squares[toSquare_r][toSquare_c] = "wQ"
            pawnPromotion = True
        elif fromPiece == "bP" and toSquare_r == 7:
            self.squares[toSquare_r][toSquare_c] = "bQ"
            pawnPromotion = True

        messageString2 = self.getMessageString(fromPiece, moveTuple, toPiece != 'e', pawnPromotion, isInCheck)

        return (messageString, messageString2, isInCheck)
